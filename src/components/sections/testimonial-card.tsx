...
export function TestimonialCard({ testimonial, locale, className }: { testimonial: PublicTestimonial; locale: string; className?: string }) {
  const name = testimonial.person_name;
  const role = resolveTranslation(testimonial.person_role_translations, locale) || testimonial.company_name;

  return (
    <article className={cn("relative flex h-full flex-col rounded-card border border-card-border bg-card-dark p-6 transition-all duration-[var(--motion-fast)] ease-[var(--ease-standard)] hover:border-card-border-hover md:p-8", className)}>
