      ...
      {LOCALES.map((locale) => {
        ...
        return (
          <button
            ...
            className={cn(
              "min-w-0 flex-1 whitespace-nowrap px-2 py-2 text-[11px] font-semibold uppercase tracking-wide transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary sm:px-3 sm:text-xs",
              active
...
function SlideCard({
  ...
  return (
    <div className="rounded-card border border-white/5 bg-background px-4 py-3.5">
...
export function AnnouncementForm({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      {/* Global language selector — one language edits every section */}
      <div>
        <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.28em] text-text-muted">
          Editing language
        </p>
        <LocaleTabs value={locale} onChange={setLocale} idPrefix="lang" />
      </div>

      {/* Enable toggle */}
      <div className="flex items-center justify-between gap-4 rounded-card border border-white/5 bg-background px-4 py-3.5">
        <div>
      ...
      <section className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <SectionHeading
            ...
            description="Messages rotate automatically on the site."
          />
          <button
            type="button"
            onClick={() => slideFields.append(emptyTr())}
            className="rounded-button border border-primary/30 bg-primary/10 px-3 py-1.5 text-xs font-semibold text-primary transition-colors hover:bg-primary/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          >
            + Add message
          </button>
        </div>
      ...
      <section className="space-y-3">
        <SectionHeading
          ...
          description="An optional button inside the bar."
        />
