...
export function FilterPills({
  ...
  return (
    <div
      className={cn(
        "flex touch-pan-x touch-pan-y overscroll-x-contain gap-3 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden",
        className
      )}
    >
      {pills.map((pill) => (
        <button
          ...
          className={cn(
            "shrink-0 rounded-md px-5 py-2.5 text-sm font-bold transition-all duration-[var(--motion-fast)] ease-[var(--ease-standard)]",
            active === pill.slug
