...
export function CookieConsentBanner({
  ...
  const visible = Boolean(settings?.banner_enabled && (consent === null || editing));
  ...
  function save(nextChoices: Record<string, boolean>) {
    const next: ConsentRecord = {
      version: CONSENT_VERSION,
      essential: true,
      analytics: nextChoices.analytics === true,
      marketing: nextChoices.marketing === true,
      ...
      {!settingsView ? (
        <>
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0"><h3 className="font-display text-base font-bold text-text-primary">{settings.title}</h3><p className="mt-1 text-xs leading-relaxed text-text-muted">{settings.text} <a href={settings.policy_url} className="text-primary underline">{t(locale, "cookiePolicy")}</a>.</p></div>
          </div>
