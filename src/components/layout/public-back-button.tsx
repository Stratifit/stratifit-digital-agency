...
export function PublicBackButton({ locale }: { locale?: string }) {
  ...
  return (
    <button
      ...
      onClick={goBack}
      className="fixed left-1 top-16 z-40 rounded-full bg-white/5 p-2 backdrop-blur-sm transition-colors duration-[var(--motion-fast)] ease-[var(--ease-standard)] hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary lg:top-20"
    >
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
