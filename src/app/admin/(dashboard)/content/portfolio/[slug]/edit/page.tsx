...
export default async function EditPortfolioPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        <h1 className="font-display text-2xl font-bold tracking-tight text-text-primary">Edit Portfolio Project</h1>
        <p className="mt-1 text-sm text-text-secondary">{slug}</p>
      </div>
      <div className="rounded-md border border-border bg-surface p-6">
        <ContentForm type="portfolio" id={slug} initial={item} />
      </div>
    </div>
  );
}
