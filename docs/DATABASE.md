...
Since migration `00050`, the `section_key` check constraint allows the live sections (`services`, `process`, `why-choose-us`, `insights`, `portfolio`, `testimonials`, `pricing`, `faq`, `acquisition`, `contact`, `acquisition-niches`, `acquisition-cta`, `tech-stack`). The removed `final-cta` and `trusted-by` keys were dropped from the constraint after their backing tables were removed (migrations `00046` and `00031`). Migration `20260826130000` adds the `related-case-studies` visibility setting for work detail pages; its seeded value is `false`, so related-project cards remain opt-in from the CMS.
...
Seeded gallery rows (27 rows across the 9 published case studies) ship in migration `00041` and `seed.sql` so work detail galleries render without a populated media library. Migration `20260823120000` (and its `seed.sql` mirror) extends each general case study to six rows; the homepage/work cards render a full cover image, except brand-design cards (`portfolio_service_links` → `brand-design`: Maison Lumière, Aura Cosmetics) which show the first four uploads as a uniform 2×2 thumbnail grid (object-cover tiles, same box size and border as every other card). The CLENQO/Aura case study is limited to four approved uploaded visuals by migration `20260826110000`. The CLENQO brand-case-study layout renders each section's media from the admin "Section images" tab when present, and otherwise falls back to the project's hero image plus gallery rows — it no longer renders generated placeholder visuals. Card images can be managed from two editors — the Portfolio Projects editor (Content tab: Category dropdown + six upload slots) and the Our Work section editor (Sections → Portfolio → Card images) — both persist to `portfolio_media` on save (delete + reinsert per project, so ordering and removals stay consistent); empty slots are dropped. Saving a project also reconciles `portfolio_service_links` to the selected category service (delete + insert). Public read access is gated by the policies from migrations `00033`/`00034`.
