...
- A successful send through `sendTemplateEmail` (manual sends, admin replies,
  ...
  - the send carried a concrete RFC 5322 Message-ID, and
  - the `from` address belongs to the synced Zoho mailbox (the `IMAP_USER`
    address or a configured sender address).
...
- `IMAP_SYNC_SENT` — enable the Sent-folder sweep (`1`/`true`).
- `IMAP_SENT_MIRROR` — enable mirroring dashboard sends into Zoho Sent.
